...
The commercial system is an eight-stage AI visibility audit pipeline. Every stage has specifications, gates and checks. Deterministic extraction is combined with LLM reasoning; guardrails block claims without measurable support.
...
| Reporting | Six languages; Ed25519 attestation | Language list, payload format, verifier |
...
This portfolio does not include the executable, model configuration, input corpus or environment lockfiles. Readers can inspect the design description but cannot reproduce a commercial audit from these files. No claim of bit-for-bit identical LLM output is made.
